...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "set to true if you have no labels and just want to "
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "Resizing shape for images (HxW). If a single int is passed, "
            "set the smallest edge of all images to this value, while keeping aspect ratio"
        ),
    )
    ...
    if args.method == "netvlad":
        if args.backbone not in [None, "VGG16"]:
            raise ValueError(
                "When using NetVLAD the backbone must be None or VGG16"
            )

        if args.descriptors_dimension not in [None, 4096, 32768]:
            raise ValueError(
                "When using NetVLAD the descriptors_dimension must be "
                "one of [None, 4096, 32768]"
            )
    ...
    # ============================================================
        ...
        if args.backbone not in [None, "VGG16"]:
            raise ValueError(
                "When using SFRS the backbone must be None or VGG16"
            )

        if args.descriptors_dimension not in [None, 4096]:
            raise ValueError(
                "When using SFRS the descriptors_dimension must be "
                "one of [None, 4096]"
            )
    ...
    # ============================================================
        ...
        if (
            args.backbone == "VGG16"
            and args.descriptors_dimension not in [64, 128, 256, 512]
        ):
            raise ValueError(
                "When using CosPlace with VGG16 the descriptors_dimension "
                "must be in [64, 128, 256, 512]"
            )

        if (
            args.backbone == "ResNet18"
            and args.descriptors_dimension not in [32, 64, 128, 256, 512]
        ):
            raise ValueError(
                "When using CosPlace with ResNet18 the descriptors_dimension "
                "must be in [32, 64, 128, 256, 512]"
            )

        if (
            args.backbone in ["ResNet50", "ResNet101", "ResNet152"]
            and args.descriptors_dimension
            not in [32, 64, 128, 256, 512, 1024, 2048]
        ):
            raise ValueError(
                f"When using CosPlace with {args.backbone} the descriptors_dimension "
                "must be in [32, 64, 128, 256, 512, 1024, 2048]"
            )
    ...
    # ============================================================
        ...
        if args.backbone not in [None, "ResNet50"]:
            raise ValueError(
                "When using Conv-AP the backbone must be None or ResNet50"
            )

        if args.descriptors_dimension not in [
            None,
            512,
            2048,
            4096,
            ...
            raise ValueError(
                "When using Conv-AP the descriptors_dimension must be "
                "one of [None, 512, 2048, 4096, 8192]"
            )
    ...
    # ============================================================
        ...
        if args.backbone not in [None, "ResNet50"]:
            raise ValueError(
                "When using MixVPR the backbone must be None or ResNet50"
            )

        if args.descriptors_dimension not in [None, 128, 512, 4096]:
            raise ValueError(
                "When using MixVPR the descriptors_dimension must be "
                "one of [None, 128, 512, 4096]"
            )
    ...
    # ============================================================
        ...
        if (
            args.backbone == "VGG16"
            and args.descriptors_dimension not in [512]
        ):
            raise ValueError(
                "When using EigenPlaces with VGG16 the descriptors_dimension "
                "must be in [512]"
            )

        if (
            args.backbone == "ResNet18"
            and args.descriptors_dimension not in [256, 512]
        ):
            raise ValueError(
                "When using EigenPlaces with ResNet18 the descriptors_dimension "
                "must be in [256, 512]"
            )

        if (
            args.backbone in ["ResNet50", "ResNet101", "ResNet152"]
            and args.descriptors_dimension not in [128, 256, 512, 2048]
        ):
            raise ValueError(
                f"When using EigenPlaces with {args.backbone} the descriptors_dimension "
                "must be in [128, 256, 512, 2048]"
            )
    ...
    # ============================================================
        ...
        if args.backbone not in [None, "ResNet50", "Dinov2"]:
            raise ValueError(
                "When using BoQ the backbone must be None, ResNet50 or Dinov2"
            )
    ...
    if args.image_size and len(args.image_size) > 2:
        raise ValueError(
            f"The --image_size parameter can only take up to 2 values, "
            f"but has received {len(args.image_size)}."
        )
